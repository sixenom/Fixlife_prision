local Utils = require 'client.toilet_utils'
...
function ScrewGame:playExitSequence()
    ...
    local crawlDuration = GetAnimDuration('promt@jail_5', 'jail_exit_ped') * 1000
    Wait(crawlDuration / 2)
    DoScreenFadeOut(500)
    Wait(500)

    local exitLoc = Config.Cells[jailCell].ExitLoc
    SetEntityCoordsNoOffset(cache.ped, exitLoc.Loc.x, exitLoc.Loc.y, exitLoc.Loc.z - 1.0, false, false, false)
