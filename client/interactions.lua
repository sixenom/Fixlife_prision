...
function CreateItemRecoveryPoint()
    if next(itemzie) then point('fixlife_prision_item', Config.ItemLoc.Loc, Config.Sayings[12], 'Fixlife_prision:client:item') end
end
...
RegisterNetEvent('Fixlife_prision:client:item', function()
    if using or isDead or not canGrab or not next(itemzie) then return end
    canGrab = false
    ...
    TriggerEvent('Fix_3dTextUi:eliminar', 'fixlife_prision_item')
